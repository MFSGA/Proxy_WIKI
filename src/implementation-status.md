...
| Shadowsocks | Inbound/outbound modules; TCP and UDP integration work exists behind `shadowsocks` feature | Inbound TCP/UDP implementation behind `shadowsocks` feature | Server currently rejects/does not implement some cipher/config variants, including `xchacha20-poly1305`. |
| AnyTLS | Inbound/outbound implementation behind `anytls`; integration tests exist | No AnyTLS server protocol in the current server protocol enum | Several Client options are parsed for compatibility but explicitly not applied by the runtime. |
